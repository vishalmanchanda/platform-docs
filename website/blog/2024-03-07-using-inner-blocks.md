...
```php
<?php
// phpcs:ignore Platform.Security.ComponentsEscape.OutputNotEscaped
...
Inner blocks behave like regular blocks, which means they also have a wrapper by default. This may not always be desired and manually removing wrapper or adding custom wrapper settings on each inner block can be tedious. That’s why Platform DevKit offers few helpers that allow you to override inner block attributes.
